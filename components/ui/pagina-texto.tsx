...
export function PaginaTexto({
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 pt-8 pb-16">
      ...
      {entradilla && <p className="mt-5 text-lg leading-relaxed text-dorado">{entradilla}</p>}

      <div className="mt-10 space-y-8 leading-relaxed text-dorado [&_a]:text-dorado [&_a]:underline [&_a]:decoration-dorado/40 [&_a]:underline-offset-4 [&_h2]:mb-2 [&_h2]:font-display [&_h2]:text-xl [&_h2]:font-semibold [&_li]:mt-1.5 [&_p+p]:mt-3 [&_ul]:list-disc [&_ul]:pl-5">
        {children}
      </div>
    </div>
  );
...
export function AvisoIncompleto() {
  return (
    <div className="rounded-xl border border-terracotta/50 bg-terracotta/15 p-5 text-sm">
      <p className="font-medium text-cream">Este texto está incompleto</p>
