...
export function PaginaTexto({
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 pt-8 pb-16">
      ...
      <h1 className="mt-8 font-display text-4xl leading-tight font-bold text-ink">{titulo}</h1>
      <span className="filete mt-5" />
      {entradilla && <p className="mt-5 text-lg leading-relaxed text-ink-soft">{entradilla}</p>}

      <div className="mt-10 space-y-8 leading-relaxed text-ink-soft [&_a]:text-ink [&_a]:underline [&_a]:decoration-gold [&_a]:underline-offset-4 [&_h2]:mb-2 [&_h2]:font-display [&_h2]:text-xl [&_h2]:font-semibold [&_h2]:text-ink [&_li]:mt-1.5 [&_p+p]:mt-3 [&_ul]:list-disc [&_ul]:pl-5">
        {children}
      </div>
    </div>
  );
